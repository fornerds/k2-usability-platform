...
type RadarChartWithTooltipProps = {
  selectedProducts: string[];
  selectedCategory: keyof CategoryData;
  onCategoryClick: (category: keyof CategoryData) => void;
  productData?: Record<string, CategoryData>;
  products?: Product[];
  responseDistribution?: Record<string, Record<keyof CategoryData, ResponseDistribution>>;
};
...
export default function RadarChartWithTooltip({
  selectedProducts,
  selectedCategory,
  onCategoryClick,
  productData,
  ...
  const getProductPath = (productId: string) => {
    ...
    const points = categories.map(cat => getPoint(data[cat.key], cat.angle));
      ...
      <div className="relative size-[240px]">
        ...
        <svg className="absolute inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 240 240">
          {selectedProducts.map((productId) => {
            ...
            if (!product || !productData[productId]) return null;
          ...
          <svg className="absolute inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 240 240">
            {selectedProducts.map((productId) => {
              ...
              return categories.map((cat) => {
                const point = getPoint(data[cat.key], cat.angle);
                const distribution = responseDistribution[productId]?.[cat.key];
                if (!distribution) return null;
                return (
                  <g
                    key={`${productId}-${cat.key}`}
                    className="cursor-pointer"
                    onMouseEnter={() => setHoveredPoint({ productId, category: cat.key, distribution })}
