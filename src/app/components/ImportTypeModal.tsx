...
import svgPaths from "../../imports/svg-88qcrpij8w";
...
function Text1({ text, additionalClassNames = "" }: Text1Props) {
  return (
    <div className={clsx("content-stretch flex gap-[12px] items-center min-h-[inherit] px-[12px] py-[20px] relative size-full", additionalClassNames)}>
      <div className="basis-0 flex flex-col justify-center leading-[0] min-h-px min-w-px not-italic overflow-ellipsis overflow-hidden relative shrink-0 text-[#2a2c2c] text-[14px] text-nowrap tracking-[-0.042px] font-semibold">
...
const mockData: TypeData[] = [
  { id: 1, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 2, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 3, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 4, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 5, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 6, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 7, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 8, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 9, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
  { id: 10, category: "아이더", name: "런닝화 피팅 평가 A1", typeId: 37 },
];

type ImportTypeModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onImport: (selectedIds: number[]) => void;
};

export default function ImportTypeModal({ isOpen, onClose, onImport }: ImportTypeModalProps) {
  ...
  const filteredData = mockData.filter(
    (item) =>
      ...
      item.typeId.toString().includes(searchTerm)
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white relative rounded-[6px] w-[800px] max-h-[90vh]" data-name="modal">
                      ...
                      <div className="content-stretch flex gap-[8px] items-center px-[12px] py-0 relative size-full">
                        ...
                        <Wrapper>
                          <g id="magnifying_glass_bold">
                            <path d={svgPaths.p1b322f00} fill="#0C0F0F" fillOpacity="0.52" id="Vector" />
                          </g>
                        </Wrapper>
                      </div>
                    </div>
              ...
              <div className="content-stretch flex flex-col items-center relative shrink-0 w-full" data-name="table">
                <div className="relative rounded-[8px] shrink-0 w-full" data-name="table">
                  <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[inherit] w-full">
                    ...
                    {paginatedData.map((item) => (
                      <div key={item.id} className="content-stretch flex items-center relative shrink-0 w-full" data-name="table row">
                        <div aria-hidden="true" className="absolute border-[0px_0px_1px] border-[rgba(12,15,15,0.13)] border-solid inset-0 pointer-events-none" />
                        ...
                        <Wrapper1 additionalClassNames="h-[56px] min-h-[56px]">
                          <Text1 text={item.category} additionalClassNames="max-w-[inherit]" />
                        </Wrapper1>
                        ...
                        <div className="content-stretch flex gap-[12px] h-[56px] items-center min-h-[56px] px-[12px] py-[20px] relative shrink-0 w-[140px]" data-name="k2 Table cell data">
                          <div className="basis-0 flex flex-col grow justify-center leading-[0] min-h-px min-w-px not-italic overflow-ellipsis overflow-hidden relative shrink-0 text-[#2a2c2c] text-[14px] text-center text-nowrap tracking-[-0.042px] font-semibold">
                            <p className="leading-[20px] overflow-ellipsis overflow-hidden">{item.typeId}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="content-stretch flex gap-[8px] items-center justify-center pb-0 pt-[20px] px-0 relative shrink-0" data-name="Pagination">
                  <button
                    ...
                    className="bg-[rgba(255,255,255,0)] content-stretch flex items-center justify-center relative rounded-[8px] shrink-0 size-[40px] disabled:opacity-50 hover:bg-gray-100 transition-colors"
                    data-name="Icon Button"
                  >
                    <PlusBold>
                      <path d={svgPaths.p3a904d00} fill="#0C0F0F" fillOpacity={currentPage === 1 ? "0.19" : "0.52"} id="Vector" />
                    </PlusBold>
                  </button>
                    ...
                    <div className="content-stretch flex h-full items-center justify-center min-w-[inherit] overflow-clip px-[8px] py-0 relative rounded-[inherit]">
                      <div className="content-stretch flex items-center justify-center relative shrink-0" data-name="input-wrap">
                        <div className="flex flex-col justify-center leading-[0] not-italic relative shrink-0 text-[#2a2c2c] text-[16px] text-nowrap tracking-[-0.08px]">
                          <p className="leading-[22px]">{currentPage}</p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col justify-center leading-[0] not-italic relative shrink-0 text-[16px] text-[rgba(12,15,15,0.29)] text-nowrap tracking-[-0.08px]">
                    <p className="leading-[22px]">/</p>
                  </div>
                  <div className="flex flex-col justify-center leading-[0] not-italic relative shrink-0 text-[#2a2c2c] text-[16px] text-nowrap tracking-[-0.08px]">
                    <p className="leading-[22px]">{totalPages}</p>
                  </div>
                  <button
                  ...
                  >
                    <PlusBold>
                      <path d={svgPaths.p3a904d00} fill="#0C0F0F" fillOpacity={currentPage === totalPages ? "0.19" : "0.52"} id="Vector" />
                    </PlusBold>
                  </button>
                </div>
              </div>
              ...
              <div className="content-stretch flex items-center justify-end pb-[24px] pt-[16px] px-[24px] relative w-full">
                <div className="content-stretch flex gap-[8px] items-center relative shrink-0" data-name="right-wrap">
                  ...
                  <button
                    ...
                    className="bg-[#1e2020] content-stretch flex gap-[2px] h-[40px] items-center justify-center min-w-[96px] px-[16px] py-0 relative rounded-[8px] shrink-0 hover:bg-[#2a2c2c] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    data-name="Label Button"
                  >
